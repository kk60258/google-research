...
class JDETracker(object):
    ...
    def update(self, pred_bboxes, pred_scores, pred_embeddings):
        """
        ...
        t1 = time.time()
        ''' Step 1: wrap data into Strack '''
        # pred now has lesser number of proposals. Proposals rejected on basis of object confidence score
        if len(pred_bboxes) > 0:
            ## bbox in absolute coordinate
            detections = [STrack(STrack.tlbr_to_tlwh(bbox), score, embedding, 30) for
                          (bbox, score, embedding) in zip(pred_bboxes, pred_scores, pred_embeddings)]
        else:
            detections = []
