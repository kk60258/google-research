...
def tf_example_from_annotations_data_frame(annotations_data_frame, label_map,
  ...
  filtered_data_frame = annotations_data_frame[
      annotations_data_frame.Class1.isin(label_map)]
  filtered_data_frame = filtered_data_frame[
    filtered_data_frame.Class2.isin(sub_label_map)]
  ...
  feature_map = {
      ...
      standard_fields.TfExampleFields.object_class_text:
          dataset_util.bytes_list_feature([
              ...
              for label_text in filtered_data_frame_boxes.Class1.to_numpy()
          ]),
      standard_fields.TfExampleFields.object_class_label:
          dataset_util.int64_list_feature(
              filtered_data_frame_boxes.Class1.map(
                  lambda x: label_map[x]).to_numpy()),
      standard_fields.TfExampleFields.object_sub_class_text:
        dataset_util.bytes_list_feature([
          six.ensure_binary(label_text)
          for label_text in filtered_data_frame_boxes.Class2.to_numpy()
        ]),
      standard_fields.TfExampleFields.object_sub_class_label:
        dataset_util.int64_list_feature(
          filtered_data_frame_boxes.Class2.map(
            lambda x: sub_label_map[x]).to_numpy()),
  ...
  if 'ConfidenceImageLabel' in filtered_data_frame_labels.columns:
    feature_map[standard_fields.TfExampleFields.
                image_class_label] = dataset_util.int64_list_feature(
                    filtered_data_frame_labels.Class1.map(
                        lambda x: label_map[x]).to_numpy())
    feature_map[standard_fields.TfExampleFields
