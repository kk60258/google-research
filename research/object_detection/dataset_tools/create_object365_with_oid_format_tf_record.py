...
copy from create_oid_tf_record.py because different ImageID format between object365 and oid

oid: remove '.jpg' from image_name
object365: contain several parent dirs before file name, ex: images/v1/patch0/object365_0000123123

use the patch id as shard index. As a result, we need to set the num_shards bigger than max patch id

Can ref create_oid_tf_record.py for other info
...
def main(_):
  ...
  output_tf_record_path_dir = os.path.dirname(FLAGS.output_tf_record_path_prefix)
  os.makedirs(output_tf_record_path_dir, exist_ok=True)
  ...
  with contextlib2.ExitStack() as tf_record_close_stack:
    ...
    for counter, image_data in enumerate(all_annotations.groupby('ImageID')):
      ...
      image_id = image_id.split(os.sep)

      image_id = image_id[2:]
      test_patch = image_id[0]
      image_id[-1] = image_id[-1] + '.jpg'
      image_id = os.sep.join(image_id)
